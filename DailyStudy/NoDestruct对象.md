...
单例模式的一种替换实现方式，在Chrome源码中推荐使用。

一般单例模式基于静态对象实现。由于C++静态对象是在程序退出后进行析构，因此很难控制构析构的流程。容易在程序退出时产生未知的问题。因此Chrome推荐使用`NoDestruct`对象，对应的对象不会进行析构函数的调用。

[代码链接]([no_destructor.h - Chromium Code Search](https://source.chromium.org/chromium/chromium/src/+/main:base/no_destructor.h;bpv=0;bpt=0))
